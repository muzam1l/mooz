import { Stack, Modal, ContextualMenu, IconButton, TooltipHost } from '@fluentui/react'
import { useCallback } from 'react'
import type { FC } from 'react'
import { useCloseButtonStyles, useModalClassnames, modalStyles, classes } from './styles'
...
const ChatPanel: FC = () => {
    ...
    const onRenderChatContent = useCallback(
        () => (
            <Messages>
                {messages.map(msg => (
                    <Message key={msg.id} title={msg.userLabel} text={msg.text} mine={msg.mine} />
                ))}
                {!messages.length && <div className={commonClasses.message}>Chat is empty</div>}
            </Messages>
        ),
        [messages],
    )
        ...
        <Stack verticalAlign="center" verticalFill>
            ...
            {!modalEnabled && !isMobile && (
                <Stack horizontal horizontalAlign="center">
                    <TooltipHost delay={0} content="Open in floating window" id="un-dock-button">
                        <HoverButton
            ...
            {!modalEnabled ? (
                <Stack.Item className={classes.vScroll}>{onRenderChatContent()}</Stack.Item>
            ) : (
                <Stack

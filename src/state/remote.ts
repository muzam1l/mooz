...
import { Stream, type IConnection, type IRemoteState, type IRoom, IPeerData } from './types'
...
export const createSocket = () => {
    ...
    const url = process.env.REACT_APP_SOCKET_URL || `${protocol}//${hostname}:${serverPort || port}`
...
export const setupLocalMediaListeners = () => {
    ...
    userStream.addEventListener("addtrack", ({ track }) => {
        ...
        connections.forEach(conn => {
            debug("adding track to peer", { track, conn, userStream, displayStream })
    ...
    userStream.addEventListener("removetrack", ({ track }) => {
        ...
        connections.forEach(conn => {
            debug("removing track from peer", { track, conn, userStream, displayStream })
    ...
    displayStream.addEventListener("addtrack", ({ track }) => {
        ...
        connections.forEach(conn => {
            debug("adding display track to peer", { track, conn, userStream, displayStream })
    ...
    displayStream.addEventListener("removetrack", ({ track }) => {
        ...
        connections.forEach(conn => {
            debug("removing display track from peer", { track, conn, userStream, displayStream })
...
export const createRemoteConnection = ({ initiator, userId, userName }: Pick<IConnection, "userId" | "userName" | "initiator">) => {
    if (!Peer.WEBRTC_SUPPORT) {
        alert("Your browser does not support WebRTC or it is disabled. Please use a WebRTC enabled browser to use this app.")
        return
    }

    const state = useRemoteState.getState()
    if (state.connections.find(c => c.userId === userId)) {
        throw new Error(`createRemoteConnection: connection with user ${userId} already exists`)
    }
    const { socket } = state
    ...
    const reRenderConnection = () => useRemoteState.setState(state => {
        return {
    ...
    userStream.getTracks().forEach(track => {
        debug("adding track to peer", track, connection.userStream)
        peer.addTrack(track, connection.userStream)
    })
    displayStream.getTracks().forEach(track => {
        debug("adding track to peer", track, connection.displayStream)
    ...
    peer.on("track", (track, stream) => {
        debug("Peer track received", { track, stream, connection })
        ...
        const streamType = stream.id === connection.metaData?.screenStreamId ? "displayStream" : "userStream"
